...
本文面向尚未使用 mjlab 的读者，说明其问题定位、分层结构、MDP 的配置方式、任务注册与训练入口，以及适用范围。技术细节以官方文档 [mujocolab.github.io/mjlab](https://mujocolab.github.io/mjlab/main/index.html) 与仓库 [mujocolab/mjlab](https://github.com/mujocolab/mjlab) 为准。应用层以 [unitree_rl_mjlab](https://github.com/tangyx96/unitree_rl_mjlab) 为贯穿示例：该仓库将 mjlab 作为依赖，在本地维护机器人资产、速度跟踪与运动模仿任务及训练脚本，并将导出的 ONNX 策略接入独立的实机部署流程。二者的职责划分是：mjlab 提供 GPU 仿真与 MDP 编排；应用仓库负责机型、任务特化以及 Train → Play → Sim2Real 中的部署环节。
...
建议先读第一、二、五部分，再按需要查阅第三、四、六部分。实现环境时以第八部分为索引，前四部分作概念对照。阅读 unitree_rl_mjlab 时，可将 `src/assets`、`src/tasks`、`scripts` 分别对应第三、四、六与八部分。
...
mjlab 的定位是：在较轻的依赖下，同时提供结构化的 MDP API 与可检查的 GPU 物理（官方文档 *Why mjlab?*）。其工程原则为：
...
实机通信、DDS 与电机 SDK 亦不在 mjlab 内。unitree_rl_mjlab 的流程为 Train → Play → Sim2Real：前两步构造 `ManagerBasedRlEnv`、调用 RSL-RL，并用 `play` 回放；第三步将 runner 导出的 `policy.onnx` 交给 C++ 部署与 `unitree_sdk2`。应用代码中凡 `from mjlab...` 的部分属于前两步；`deploy/` 已脱离该框架。

---

## 二、两层架构

可将 mjlab 视为上下两层，由环境类连接。
...
- **仿真层**描述世界的组成、受力与感知，产物为 GPU 上的并行物理。
- **管理器层**描述智能体的观测、动作、终止条件与随机化，产物为一个 MDP。
- **`ManagerBasedRlEnv`** 按固定顺序调用各 manager 并推进物理。
- **RSL-RL** 只依赖向量化环境接口，不依赖 MJCF 的具体写法。

修改 PD 增益、碰撞与传感器属于仿真层；修改奖励权重、观测噪声与扰动间隔属于管理器层。二者混写会使环境重新变为单体脚本。unitree_rl_mjlab 遵循该划分：`src/assets` 对应仿真层，`src/tasks` 对应 manager 字典，不修改 `ManagerBasedRlEnv` 的步进顺序。

环境规格集中于扁平 dataclass **`ManagerBasedRlEnvCfg`**：场景、仿真步长，以及各 manager 的 term 字典。配置不采用深层 `__post_init__` 继承；拼写错误的字段名在构造时引发 `TypeError`。
...
一份 `ActuatorCfg` 表示**同一硬件模型**作用于一组关节（刚度、阻尼、`armature` 等在组内均匀）。增益不同时应按关节组拆分配置，而不是在单份配置中使用 per-joint 字典。例如 Go2 将髋、大腿、小腿分为三个 `BuiltinPositionActuatorCfg`（小腿刚度与力矩上限较高），并用 `CollisionCfg` 指定参与接触的 geom、足端 `condim` 与摩擦。上述内容写在应用仓库的 `get_*_robot_cfg()` 中；mjlab 仅提供配置类型。
...
1. **任务注册表。** 每个任务对应一对配置：`ManagerBasedRlEnvCfg` 与 `RslRlOnPolicyRunnerCfg`。`register_mjlab_task` 用字符串 id 绑定二者，并另存 `play_env_cfg`（关闭训练用随机化、延长回合）供评估。官方内置任务多用 `Mjlab-{Category}-{Terrain}-{Robot}`；应用仓库可自行命名，例如 `Unitree-G1-Flat`、`Unitree-G1-Tracking-No-State-Estimation`，只要不与已注册 id 冲突。可选参数 `runner_cls` 默认为 `MjlabOnPolicyRunner`。应用仓库常将其替换为在保存 checkpoint 时导出 ONNX 的子类（速度跟踪与模仿任务中的 `*OnPolicyRunner`）。
2. **`RslRlVecEnvWrapper`。** 将观测字典转换为 RSL-RL 的 TensorDict；合并 `terminated` 与 `truncated` 为 `dones`，并将超时写入 `extras` 以便正确自举；可按配置裁剪动作。构造时调用 `env.reset()`，因为 RSL-RL 在收集 rollout 前不自行 reset。应用仓库若自备 `scripts/train.py`，仍是先构造 `ManagerBasedRlEnv`，再包装该层，然后交给 runner。
...
`--num-envs` 为顶层便捷参数，等价于覆盖 `env.scene.num_envs`（unitree_rl_mjlab 的脚本常写成 `--env.scene.num-envs=4096`）。环境与算法的配置树可通过 tyro 用点分路径覆盖。CLI 使用连字符（`--num-envs`）而非下划线；布尔标志须显式给出 `True` 或 `False`（例如 `--agent.resume True`），以便与 W&B sweep 兼容。多 GPU 使用 `--gpu-ids`；官方 CLI 因 tyro 配置而采用 Python 字面量，例如 `--gpu-ids "[0, 1]"`。
...
`--agent` 可选 `trained`、`zero`、`random`。新任务宜先以 zero 与 random 检查奖励符号、终止条件与动作缩放。查看器可选原生 MuJoCo（`NativeMujocoViewer`）或浏览器端 Viser（`ViserPlayViewer`）。应用仓库的 `play.py` 通常直接构造这两类，而不另实现可视化后端。

日志目录形如 `logs/rsl_rl/{experiment_name}/{timestamp}/`，包含策略 checkpoint 以及 `params/env.yaml`、`params/agent.yaml`。带 ONNX 导出的 runner 还会在同目录写入 `policy.onnx`（及元数据），供部署读取关节顺序与 PD 增益。

### 6.3 参考任务

mjlab 提供三类参考任务，应用仓库大多在此基础上特化：

- **速度跟踪**：跟踪平面速度指令。官方任务如 `Mjlab-Velocity-Flat-Unitree-G1`；unitree_rl_mjlab 在同一套 manager 上注册 `Unitree-Go2-Flat`、`Unitree-G1-Flat`、`Unitree-H1_2-Flat` 等。
- **运动模仿**：`MotionCommand` 从 `.npz` 读入参考。官方使用运动资产 registry；应用仓库常用 `--motion_file` 指向本地 npz（CSV 需先按控制频率重采样）。
- **操作**：例如举升类 command。

Cartpole 教程给出最小可运行 MDP。文档 *Research* 页列出 PAL、Upkie、灵巧手等外部项目。建议将 `mjlab` 作为依赖引入（unitree_rl_mjlab 使用 `mjlab==1.2.0`），任务与 MJCF 放在应用仓库的 `src/`，而不是 fork mjlab 本体。若 MDP 与官方一致，只需 patch `make_velocity_env_cfg()`（ANYmal C 示例）；若需修改奖励、观测或 ONNX 导出逻辑，可将 `velocity_env_cfg.py` / `tracking_env_cfg.py` 与 `mdp/` 放入应用仓库，对 `mjlab.envs.mdp` 执行 `import *` 后再叠加本地 term。底层仍是同一套 `ManagerBasedRlEnvCfg`。
...
明确不作为目标的包括：照片级仿真、USD 资产管线、跨 PhysX/Newton 的同一套环境代码、将可微物理作为一等能力、将高保真视觉作为默认观测，以及实机总线与 SDK。Sim2Real 部署位于应用仓库的 `deploy/`，而不位于 `mjlab.envs`。
...
6. 实现：[Cartpole 教程](https://mujocolab.github.io/mjlab/main/source/tutorials/cartpole.html)；仅换机型见 [anymal_c_velocity](https://github.com/mujocolab/anymal_c_velocity)；多机型与部署见 [unitree_rl_mjlab](https://github.com/tangyx96/unitree_rl_mjlab)
...
实现阶段的主要问题通常不是单个 Manager 的字段名，而是文件组织、CLI 如何发现任务，以及 XML、配置与注册三者如何衔接。逐步教程见 [Cartpole](https://mujocolab.github.io/mjlab/main/source/tutorials/cartpole.html)；仅更换机型见 [anymal_c_velocity](https://github.com/mujocolab/anymal_c_velocity)；多机型、自定义 MDP 与 ONNX 部署见 [unitree_rl_mjlab](https://github.com/tangyx96/unitree_rl_mjlab)。
...
| **C. 应用仓库依赖 mjlab** | 需修改奖励与观测、增加机型、导出 ONNX、接入实机 | unitree_rl_mjlab：`src/assets`、`src/tasks`、`scripts/` |
...
- 物理与资产 → `entity` / `actuator` / `asset_zoo`，或应用仓库的 `src/assets/robots/`
- MDP 项 → `mjlab.envs.mdp`，或应用仓库 `src/tasks/<name>/mdp`（对前者 `import *` 再叠加）
...
2. **显式 import。** 训练脚本执行 `import mjlab.tasks` 与 `import src.tasks`。后者通过 `mjlab.utils.lab_api.tasks.importer.import_packages` 递归导入含子包的目录；各机器人配置包的 `__init__.py` 调用 `register_mjlab_task`。unitree_rl_mjlab 采用该方式，因此可以没有 `mjlab.tasks` entry point，但中间目录必须包含 `__init__.py`，且应将纯工具包（如 `.mdp`）列入黑名单，以免将 term 模块当作任务包导入。
...
`play=True` 应关闭训练用随机化（扰动、观测 corruption）并延长回合。官方 id 多用 `Mjlab-` 前缀；应用仓库使用 `Unitree-` 等前缀即可，但须避免与已注册 id 冲突。
...
- `cfg.scene.entities["robot"]` ← `EntityCfg`（`asset_zoo` 或本地 `get_g1_robot_cfg()`）
...
路径 B 到此为止，工厂仍来自 `mjlab.tasks.velocity`。路径 C 在应用仓库保留同名工厂，以便修改奖励或观测；`mdp/__init__.py` 先执行 `from mjlab.envs.mdp import *`，再导出本地的 rewards、observations、commands。动作仍使用 `JointPositionActionCfg`，随机化仍使用 `mjlab.envs.mdp.dr`，PPO 仍使用 `mjlab.rl` 中的 dataclass。目录结构可为：
...
关节名、PD、`armature` 与 `CollisionCfg` 应集中在 `*_constants.py`（或等价模块）中。`spec_fn` 用 `MjSpec.from_file` 加载 XML，网格经 `update_assets` 写入 `spec.assets`。传感器名称与观测中的 `sensor_name` 必须带实体前缀。

路径 B 可通过 uv 的 entry point 注册任务。路径 C 在 `setup.py` 或 `pyproject.toml` 中声明 `mjlab==…`，由训练入口自行 `import src.tasks`。缺少 build backend 时，可编辑安装与入口脚本可能无法正确注册。
...
2. `list-envs` 或应用仓库的 `scripts/list_envs.py` 列出目标 `task_id`。
...
7. 再提高并行规模。若需上真机，先确认 checkpoint 目录中已有 `policy.onnx`，再进入应用仓库的 `deploy/`；该步骤不再调用 mjlab。
...
| Entity、PD、碰撞 | `asset_zoo` 或 `src/assets/robots/*/…_constants.py` |
| 训练 CLI 与配置覆盖 | `mjlab/scripts/train.py` 或应用仓库 `scripts/train.py` |
| 任务如何进入注册表 | `register_mjlab_task`；entry point 或 `import_packages` |
| 策略如何离开仿真 | `mjlab.rl.exporter_utils`、应用仓库的 `*OnPolicyRunner.save` |

场景对应「世界中有什么」，manager 字典对应「智能体与世界如何交互」，`register_mjlab_task` 对应「CLI 如何索引该任务」。应用仓库会再次使用框架提供的类型：`EntityCfg`、`BuiltinPositionActuatorCfg`、`CollisionCfg`、`SceneCfg`、传感器、`ManagerBasedRlEnvCfg` 的各字典、`JointPositionActionCfg`、`UniformVelocityCommandCfg` / `MotionCommandCfg`、`dr.*` 事件，以及 RSL-RL 的 runner 与 wrapper。应用仓库额外维护的是机型常量、对工厂的 patch，以及 mjlab 范围之外的 ONNX 与实机进程。
...
cfg = ManagerBasedRlEnvCfg(
    ...
    scene=...,              # 地形、实体、传感器、num_envs
    sim=...,                # timestep、求解器、积分器
    observations={...},     # 可含 actor / critic 两组
    actions={...},
    rewards={...},
    terminations={...},
    events={...},           # 默认含 reset_scene_to_default
    commands={...},
    curriculum={...},
    metrics={...},
)
...
`scene` 描述物理世界，各 manager 字典描述 MDP，`decimation` 给出策略时钟与物理时钟之比。实现新任务时，可据此判断应修改哪一层。
