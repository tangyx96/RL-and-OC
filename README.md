...
Bertsekas 提供 Bellman / MPC / 策略梯度骨架；Zero-Order 用随机搜索统一 TO 与 RL；SQP 组用数值优化统一 iLQR、DDP 与结构利用型 QP；`clean-rl/` 把策略梯度与 off-policy Actor-Critic 落到推导上；`simulators/` 按仿真器与训练库归档；其中 `mjlab/` 对应 GPU 并行仿真上的 manager-based 机器人 RL 环境，`rsl_rl/` 对应接到这些环境上的 on-policy 训练循环。
