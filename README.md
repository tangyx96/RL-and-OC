...
| [`policy-learning/`](policy-learning/) | PPO / SAC / FlashSAC；Diffusion Policy；Flow Matching（π₀）；VLA；WAM | [`ppo_notes.md`](policy-learning/ppo_notes.md)、[`sac_notes.md`](policy-learning/sac_notes.md)、[`flashsac_notes.md`](policy-learning/flashsac_notes.md)、[`diffusion_policy_notes.md`](policy-learning/diffusion_policy_notes.md)、[`flow_matching_notes.md`](policy-learning/flow_matching_notes.md)、[`openvla_notes.md`](policy-learning/openvla_notes.md)、[`wam_notes.md`](policy-learning/wam_notes.md) |
...
Bertsekas 提供 Bellman / MPC / 策略梯度骨架；Zero-Order 用随机搜索统一 TO 与 RL；SQP 组用数值优化统一 iLQR、DDP 与结构利用型 QP；`policy-learning/` 把策略梯度、off-policy Actor-Critic 与模仿学习（Diffusion Policy、Flow Matching、VLA、WAM）落到推导上；`simulators/` 按仿真器与训练库归档；其中 `mjlab/` 对应 GPU 并行仿真上的 manager-based 机器人 RL 环境，`rsl_rl/` 对应接到这些环境上的 on-policy 训练循环。
